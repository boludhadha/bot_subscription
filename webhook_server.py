...
async def create_temporary_invite_link(bot, chat_id, expire_seconds=14400, member_limit=1):
    try:
...
def verify_paystack_webhook(request_body, signature):
    hash = hmac.new(PAYSTACK_SECRET_KEY.encode(), request_body,
...
def paystack_webhook():
    try:
        ...
        if verify_paystack_webhook(raw_body, signature) == False:
            logger.info("Unauthorized request to paystack webhook from {} blocked".format(
                request.origin))
            abort(405)
        ...
        if payload and payload.get('event') == 'charge.success':
            ...
            if payment_reference:
                ...
                if payment_verification.get('status'):
                    ...
                    invite_link = asyncio.run(create_temporary_invite_link(
                        bot_instance, TELEGRAM_GROUP_ID))

                    amount_in_naira = amount // 100

                    # notification with invite link
                    asyncio.run(send_notification(bot_instance, telegram_chat_id,
                    ...
                    logger.info('Payment received - Reference: %s, Amount: %s %s',
                                payment_reference, amount_in_naira, currency)
