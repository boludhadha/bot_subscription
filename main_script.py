...
from db import create_tables, add_subscription, add_payment_session, get_expired_subscriptions, remove_subscription, update_payment_session_status
...
def initiate_payment(amount, email, reference, telegram_chat_id, subscription_type, username):
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True, input_field_placeholder="Select an option below to continue👇🏽")
...
async def select_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if selected_plan in subscription_plans:
        ...
        payment_response = initiate_payment(amount, email, reference, telegram_chat_id, subscription_type, username)
        ...
        if payment_response.get('status'):
            ...
            await query.edit_message_text(text="Failed to initiate payment. Please try again later.")
    else:
        logging.error(f"Invalid plan selected: {selected_plan}")
        await query.edit_message_text(text="Invalid subscription plan selected. Please try again.")
...
async def check_subscription_expiry(context: ContextTypes.DEFAULT_TYPE):
    ...
    for subscription in expired_subscriptions:
        ...
        await bot_instance.send_message(chat_id=telegram_chat_id, text="Your subscription has expired. You will be removed from the group.")
        # Remove user from the group
        await bot_instance.ban_chat_member(chat_id=TELEGRAM_GROUP_ID, user_id=telegram_chat_id)
        remove_subscription(telegram_chat_id)
        logging.info(f"User {telegram_chat_id} removed from group due to expired subscription")
...
if __name__ == '__main__':
    ...
    job_queue.run_repeating(check_subscription_expiry, interval=datetime.timedelta(seconds=200), first=0)  # Check every 5 seconds for testing
